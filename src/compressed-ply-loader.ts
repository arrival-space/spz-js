...
const dataTypeMap = new Map<string, TypedArrayConstructor>([
    ['char', Int8Array],
    ['uchar', Uint8Array],
    ['short', Int16Array],
    ['ushort', Uint16Array],
...
type TypedArrayConstructor = Int8ArrayConstructor | Uint8ArrayConstructor | Int16ArrayConstructor |
...
const isCompressedPly = (elements: HeaderElement[]) => {
...
class SplatCompressedIterator {
    ...
    constructor(gsplatData: {
        compressedData: CompressedPlyData;
        chunkSize: number;
        shBands: number;
    }, p: Vec3, r: Quat, s: Vec3, c: Vec4, sh: Float32Array | null) {
        const unpackUnorm = (value: number, bits: number) => {
            const t = (1 << bits) - 1;
            return (value & t) / t;
        };

        const unpack111011 = (result: Vec3, value: number) => {
            result.x = unpackUnorm(value >>> 21, 11);
            result.y = unpackUnorm(value >>> 11, 10);
            result.z = unpackUnorm(value, 11);
        };

        const unpack8888 = (result: Vec4, value: number) => {
        ...
        const unpackRot = (result: Quat, value: number) => {
...
const decompress = (compressedData: CompressedPlyData, elements: HeaderElement[]) => {
    const members = [
        'x', 'y', 'z',
...
export async function loadCompressedPly(stream: ReadableStream<Uint8Array>) {
    ...
    const evalStorageSize = (count: number) => {
    ...
    // allocate result
    const result: CompressedPlyData = {
    ...
    const read = async (buffer: ArrayBufferLike, length: number) => {
