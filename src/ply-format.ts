...
function getFieldIndex(fields: Map<string, number>, name: string): number {
...
export async function loadPly(stream: ReadableStream<Uint8Array>): Promise<GaussianCloud> {
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        for (let j = 0; j < shDim; j++) {
            for (let c = 0; c < 3; c++) {
                result.sh[(i * shDim + j) * 3 + c] = dataView.getFloat32(
                    (baseOffset + shIdx[j + c * shDim]) * floatSize,
                    true
                );
            }
        }
...
export function serializePly(data: GaussianCloud): ArrayBuffer {
