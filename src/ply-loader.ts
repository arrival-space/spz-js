...
/* ────────────────────────────── helpers ───────────────────────────────── */
function getFieldIndex(map: Map<string, number>, name: string): number {
...
function byteSizeForType(type: string): number {
  switch (type) {
    case 'float':
    case 'int':
    case 'uint':
...
export async function loadPly(
  ...
  let   off         = 0;
  for (const c of headerChunks) { stitched.set(c, off); off += c.length; }
  ...
  let formatOK   = false;
  let numPoints  = 0;
  const propDefs: { name: string; type: string; offsetBytes: number }[] = [];
  let strideBytes         = 0;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line.startsWith('element')) {
      ...
      if (elem === 'vertex') numPoints = Number(count);
      continue;
    }
  ...
  if (!formatOK)  throw new Error('[PLY] Only binary_little_endian supported');
  if (!numPoints) throw new Error('[PLY] Vertex count missing or zero');

  /* ---------- 4. Fast lookup tables ------------------------------------- */
  const fieldOffsetMap = new Map<string, number>();
  propDefs.forEach(({ name, offsetBytes }) =>
    fieldOffsetMap.set(name, offsetBytes / 4)   // convert to float index
  );
  ...
  while (cursor < totalBinaryBytes) {
    const { value, done } = await reader.read();
    if (done) throw new Error('[PLY] Truncated binary section');
